                      ...
                      <div className="relative mt-3">
                        ...
                        <div className="px-4 py-2 min-h-[60px]">
                          ...
                          <div className="mt-4 pt-3 border-t border-indigo-100 flex justify-between items-center">
                             <div className="text-xs font-semibold text-slate-500">Author: <strong className="text-indigo-900">{solutions[solIndex].user_name || 'RankResult AI Engine'}</strong></div>
            ...
            >
              ...
              <div className="p-4 border-t border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div className="text-xs text-slate-400 font-semibold">
                  Generated by <span className="text-indigo-600 font-bold">{solutions[solIndex].user_name || 'RankResult AI Engine'}</span>
                </div>
                <button
                  onClick={() => setShowPopup(false)}
      ...
      <div className="relative z-10 flex flex-col items-center max-w-md w-full px-6 text-center">
        ...
        <div className="bg-white/80 border border-slate-200 shadow-xl rounded-2xl p-4 w-full backdrop-blur-md">
          ...
          <p className="text-sm text-slate-600 font-medium leading-relaxed">
            RankResult's AI engine analyzes millions of data points to predict your normalized score and all-India rank with high precision.
          </p>
        </div>
      </div>
    </div>
...
export default function ResultPage() {
  ...
  const handleShare = async () => {
    const message = `I scored ${data?.result?.score} marks in ${data?.result?.subject || 'the exam'} on RankResult. Check your own score prediction and compare with the official exam experience.`;
    const shareUrl = 'https://rankresult.in/';
    const shareData = {
      title: 'My RankResult Score Card',
      text: message,
      url: shareUrl
    };
    ...
    try {
      ...
      const file = new File([blob], `RankResult_Scorecard_${data?.result?.roll_number || 'scorecard'}.png`, { type: 'image/png' });
  ...
  const pageDesc = `Check ${result.candidate_name || 'your'}'s score: ${result.score} marks, Rank #${rank?.rank || result.rank}, Percentile ${(rank?.percentile || result.percentile || 0).toFixed ? (rank?.percentile || result.percentile || 0).toFixed(1) : '—'}% in ${result.subject || 'the exam'}. Download score card from RankResult.`;
